...
1. **Shared, quote-aware CSV codec (`src/lib/bulk/csv.ts`).**
...
2. **`export/route.ts`'s `generateCSV` uses the new codec; add the missing export test file.**
...
3. **Fix bulk-operation history listing — the two bugs named above.**
...
4. **Import route: a malformed row fails alone (AC4, first half).**
...
5. **Import route: canonical per-row outcomes (AC4, second half).**
...
6. **Pre-parse size guard (AC6).**
   New `src/lib/bulk/constants.ts` exporting `MAX_IMPORT_BYTES = 5 * 1024 * 1024` (5 MB — the
   figure the design mockup states) — imported by both the route and the client component so the
   number the UI quotes can never drift from what the server enforces. At the very top of
...
7. **Post-batch version snapshot (AC7) — per `docs/decisions/008-bulk-import-write-path.md`.**
   ...
   user.email ?? user.id, p_description: <summary of the counts>, p_change_type: "bulk_import"
   ...
   `create_content_version` RPC call with the site's id and `change_type: "bulk_import"`; an
   import where every row is `skipped`/`failed` (nothing applied) triggers zero calls.

8. **Round-trip integration test (AC3) — the criterion the story names explicitly.**
...
9. **Wire `BulkOperations` into `SiteDetailView.tsx` (the dark-feature fix).**
...
10. **Rebuild the tab bodies from `src/components/ui/`, per the design doc.**
...
- Do not add a content-length validation rule (see task 5) — no such limit exists in the schema
  today and no acceptance criterion asks for one.
...
- `docs/decisions/008-bulk-import-write-path.md` — already written, travels with this branch.
