...
Research: `docs/research/s17-cluster-engine.md` — read it first; this plan does not repeat it.
Design: `docs/designs/s17-cluster-engine.md` — marketing surface, no `SkyBackground`/`useLenis`,
reused sections inventoried there. This plan does not repeat that spec either.
Architecture: `docs/decisions/012-cluster-content-engine.md` (data + route + JSON-LD engine shape),
`docs/decisions/013-lighthouse-ci-thresholds.md` (AC 7's numeric gate).

## Target story

**s17-cluster-engine — comparison pages that rank.** Complexity 3. No dependencies. Gates
`s18-stack-recipes` and `s19-audience-pages`.
...
- [ ] Page content lives in typed, validated data, so adding a competitor requires no new route
  code.
...
1. [ ] **Typed comparison data + validator.** Add `src/content/alternatives.ts`: a closed union
   ...
   runtime validator (same pattern as `plan-types.ts`'s `is*` guards) that throws at module load
...
6. [ ] **`llms.txt`.** New route handler `src/app/llms.txt/route.ts`, `text/plain`, generated from
   ...
   and have all three call sites (`sitemap.ts`, `robots.ts`, `llms.txt/route.ts`) import it — a
...
- Do not touch `src/app/api/blog/generate/route.ts` or `cron/generate-blog-post` — separate
  concern, and its insert-with-`status:"published"` behavior must not be copied into this story's
  content pipeline (it is authored TS data reviewed in a PR, not cron-generated).
- Do not import `SkyBackground`, `useLenis`, `framer-motion`, or any `three`/R3F module into
...
The entire "`s18`/`s19` need no new route code" promise (`stories.md`, and `s19`'s own agentic
notes: *"if this needs new route code, `s17` was built wrong"*) rests on task 4's JSON-LD builders
being genuinely generic — parameterized over plain `{question, answer}[]` / `{label, href}[]`
shapes with zero `ComparisonEntry`-specific typing leaking into their signatures. If task 4 is
built by inlining JSON-LD construction directly in the page component instead of as reusable
functions in `src/lib/seo/json-ld.ts`, `s18` and `s19` will each reimplement it, and the "s17 was
...
thresholds (or the page's composition) need revisiting, not evidence to relax the gate silently.
...
- `docs/decisions/012-cluster-content-engine.md`, `013-lighthouse-ci-thresholds.md` (already
  written, travel with this branch)

## Test strategy

Jest + Testing Library for: the data validator (task 1/7), route rendering and `notFound()`
behavior (task 2/3), JSON-LD-matches-rendered-content (task 4), sitemap merge (task 5), the
`llms.txt` route (task 6) — all colocated in `__tests__/` per repo convention. Lighthouse CI
(task 8) is its own verification, run locally once and then continuously in CI; it is not a Jest
test.

## Definition of Done

Repo DoD (lint, type-check, build, test green; single PR; review passed) plus:
- All 5 competitor pages render with distinct, validated content; unknown slug 404s on the
  marketing surface.
- `sitemap.ts` and `llms.txt` both list all 5 URLs with zero manual sitemap edits beyond task 5's
  one-time block.
- JSON-LD on each page validates against schema.org's required fields for all three types, and
  the FAQ schema's questions match what's rendered.
- `npm run lighthouse:ci` passes locally against a production build for all 6 URLs.
