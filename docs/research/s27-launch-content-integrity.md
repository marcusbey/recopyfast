...
The forward migration `20260924030000_content_page_path.sql` adds the nullable path/index, `save_staging_content_atomic`, and `publish_staging_content_with_attributes_atomic`. The existing two-column publish function delegates to the new implementation, preserving its signature without copying transaction logic. The new publish RPC accepts an optional page path so confirmation matches the scoped preview; pathless clients and the old RPC remain site-wide. Restore stages only snapshot attributes with different published values. New RPC grants follow the service-role-only precedent with pinned search paths. Apply the migration before the application; rollback the application first and retain the additive migration.
