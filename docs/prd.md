...
2. **A/B testing: parked or in?** → **Re-enabled.** It was pulled from the launch on
   2026-08-03 and is back in the perimeter. Stories `s11-ab-run-test` and `s12-ab-results`.
...
4. **Impression definition** → ≥ 50% of the section in viewport for ≥ 1 continuous second,
   one impression per section per page view, no visitor identifier. Story
   `s09-section-impressions`.
5. **Conversion definition for A/B** → a click on a tracked CTA within the same page view as
   an impression of the tested section. Story `s12-ab-results` states it as an acceptance
   criterion and declares `s09` a dependency for the impression half.

Resolved at `/ks-architect`:

6. **Realtime transport.** Native `WebSocket` for the embed, Socket.io retained for the
   first-party dashboard, `server/` serving both on separate paths. HTTP stays authoritative;
   realtime broadcasts and must be provably additive.
