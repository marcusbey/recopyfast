...
| Refused — size limit | Fires the instant a file is chosen, **before any parsing or upload** — `Alert variant="destructive"` under the file control: "This file is 12.4 MB. The import limit is 5 MB — split it into smaller files and try again." The file input clears itself; the `Button` never becomes enabled for this file. This is a distinct state from "failed after trying," because the story requires the refusal to happen before parsing starts |
