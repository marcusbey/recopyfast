...
export function PresenceIndicator({
  ...
  const editingUsers = relevantPresence.filter(
    (p) => p.elementId === elementId && p.cursorPosition !== undefined,
  );

  // Get users viewing but not editing
  const viewingUsers = relevantPresence.filter(
    (p) =>
      !elementId || p.elementId !== elementId || p.cursorPosition === undefined,
  );
...
export function CursorIndicator({ presence, position }: CursorIndicatorProps) {
  ...
  return (
    ...
    >
      ...
      {/* User label */}
      <div
        className={`absolute top-0 left-2 px-2 py-1 text-xs text-white rounded whitespace-nowrap ${getUserColor(presence.userId)}`}
      >
        {presence.userName || presence.userEmail}
      </div>
    </div>
