...
const mockCreateClient = createClient as jest.MockedFunction<
  typeof createClient
>;
const mockCreateServiceClient = createServiceRoleClient as jest.MockedFunction<
...
describe("/api/sites/register - POST", () => {
  beforeEach(() => {
    ...
    mockCreateClient.mockResolvedValue(
      mockAuthClient as unknown as ReturnType<typeof createClient>,
    );
  ...
  it("should successfully register a new site", async () => {
    ...
    expect(mockServiceClient.select).toHaveBeenNthCalledWith(
      2,
      "id, domain, name, created_at, api_key",
    );
