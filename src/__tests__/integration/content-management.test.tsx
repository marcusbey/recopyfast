...
describe("Content Management Integration", () => {
  ...
  describe("Content Retrieval API Flow", () => {
    ...
    it("should filter content by language and variant", async () => {
      ...
      const spanishResponse = await mockFetch(
        `/api/content/${siteId}?language=es&variant=premium`,
      );
  ...
  describe("Content Management UI Integration", () => {
    ...
    const MockContentManager = ({ siteId }: { siteId: string }) => {
      ...
      const [editingElement, setEditingElement] = React.useState<string | null>(
        null,
      );
      ...
      const fetchContent = async () => {
        ...
        try {
          const response = await fetch(
            `/api/content/${siteId}?language=${selectedLanguage}`,
          );
          ...
          <div data-testid="content-list">
            {content.map((element) => (
              <div
                ...
                data-testid={`content-element-${element.element_id}`}
              >
                <div data-testid="element-selector">
                  {element.selector as string}
                </div>
                ...
                <div data-testid="element-language">
    ...
    it("should load and display content elements", async () => {
      ...
      expect(
        screen.getByTestId("content-element-hero-title"),
      ).toBeInTheDocument();
      expect(
        screen.getByTestId("content-element-hero-subtitle"),
      ).toBeInTheDocument();
    ...
    it("should handle content editing workflow", async () => {
      ...
      expect(
        screen.getByTestId("content-element-hero-title"),
      ).toHaveTextContent("Welcome to Our Site");
    });
  ...
  describe("Complete Content Management Workflow", () => {
    it("should handle full content lifecycle", async () => {
      ...
      const frenchResponse = await mockFetch(
        `/api/content/${siteId}?language=fr&variant=premium`,
      );
