  ...
  describe("Component Error Handling", () => {
    ...
    it("should catch and handle component errors", () => {
      ...
      expect(screen.getByTestId("error-message")).toHaveTextContent(
        "Component crashed intentionally",
      );
      ...
      if (error) {
        return (
          <div data-testid="error-state">
            ...
            <button onClick={() => fetchData(true)} data-testid="retry-button">
              Retry
            </button>
          </div>
        );
    ...
    it("should recover from network errors", async () => {
      ...
      server.use(
        http.get("/api/content/:siteId", () => {
          ...
          return HttpResponse.json([{ id: "1", content: "Test content" }]);
        }),
      );
  ...
  describe("Graceful Degradation", () => {
    ...
    it("should handle WebSocket connection failures", () => {
      const MockWebSocketComponent = () => {
        ...
        return (
          <div data-testid="websocket-status">WebSocket Status: {status}</div>
        );
      };
