...
describe("Rate Limiter", () => {
  ...
  describe("createRateLimitConfig", () => {
    it("should create config with predefined limits", () => {
      ...
      expect(config.maxRequests).toBe(
        RATE_LIMIT_CONFIGS.USER_GENERAL.maxRequests,
      );
    });

    it("should handle different identifier types", () => {
      const userConfig = createRateLimitConfig(
        "user123",
        "user",
        "USER_GENERAL",
      );
      const ipConfig = createRateLimitConfig("192.168.1.1", "ip", "IP_GENERAL");
      const apiConfig = createRateLimitConfig(
        "api_key_123",
        "api_key",
        "API_KEY_DEFAULT",
      );
  ...
  describe("RATE_LIMIT_CONFIGS", () => {
    ...
    it("should have appropriate limits for different contexts", () => {
      ...
      expect(RATE_LIMIT_CONFIGS.IP_GENERAL.maxRequests).toBeGreaterThan(
        RATE_LIMIT_CONFIGS.USER_GENERAL.maxRequests,
      );

      // Enterprise API keys should have highest limits
      expect(RATE_LIMIT_CONFIGS.API_KEY_ENTERPRISE.maxRequests).toBeGreaterThan(
        RATE_LIMIT_CONFIGS.API_KEY_DEFAULT.maxRequests,
      );
    });
  });
});
