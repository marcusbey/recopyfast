/**
 * A-2 — the site token is a published credential, and its Origin pin is optional.
 *
 * `data-site-token` ships as a plain HTML attribute on every customer page
 * (src/lib/sites/embed-script.ts:53), so it is readable with View Source. The
 * single thing that makes a published credential safe is the check that it was
 * presented from the registered domain — and that check is written as
 *
 ...
 * A browser cannot omit it; curl omits it by default. So the guard is skipped
 ...
 * exist, but every id not yet discovered is seedable, and the widget then serves
 * the attacker's `published_content` to every visitor.
 *
 * These are `test.failing`: each passes while the hole is open and fails the
 * moment a missing Origin is treated as a refusal rather than as a pass.
 */
...
describe("authorizeSiteRequest Origin enforcement", () => {
  ...
  // GUARD for the headerless refusal below. `test.failing` passes on ANY
  // throw — a bad import, a stale mock, a typo — so each marked test needs an
  // unmarked sibling driving the identical call and asserting something true
  // either way. Here: the call reaches the real code path and looks the site
  // up, so a marked pass means the Origin decision was wrong, not that the
  // fixture broke.
  ...
  test.failing(
    "refuses a request that sends neither Origin nor Referer",
    async () => {
      // curl, a server-side script, anything that is not a browser. The token
      // came from View Source; nothing else is needed.
      await expect(
        authorizeSiteRequest({
          siteId: SITE_ID,
          token: publishedSiteToken(),
  ...
  // GUARD for the unparseable-Origin refusal below.
  ...
  test.failing(
    "refuses a request whose Origin header is present but unparseable",
    async () => {
      // `parseOrigin` returns null for anything `new URL()` rejects
      // (site-auth.ts:33-40), and null lands in the same permissive branch. So
      // sending `Origin: null` — which browsers really do send for sandboxed
      // and some redirected requests — is treated as "no Origin to check".
      await expect(
        authorizeSiteRequest({
          siteId: SITE_ID,
          token: publishedSiteToken(),
  ...
  // GUARD for the authorizeSiteOrigin refusal below.
  ...
  test.failing(
    "authorizeSiteOrigin refuses a headerless caller too",
    async () => {
      // The same optional-guard shape at site-auth.ts:218. This is the function
      // behind the content preflight, so a refusal here is what keeps a
...
describe("POST /api/content/[siteId] without an Origin header", () => {
  ...
  // GUARD for the seed-write refusal below: the handler runs to completion and
  // returns a response for this exact request, whatever it decides.
  ...
  test.failing("refuses an unauthenticated seed write", async () => {
    // The id is computed from the victim's own markup, so an attacker can name
    // an element the widget has not reached yet — a page never visited, an
    // element added since the last scan — and own its content before discovery
    // does. `hydrateStoredContent` serves it to every visitor thereafter.
    const response = await POST(
      headerlessPost({
        "rcf-not-yet-discovered": {
          selector: "h1",
    ...
    // Asserted on the write, not on the status: this route answers 200 while
    // persisting the attacker's row.
    expect(upsert).not.toHaveBeenCalled();
    expect(response.status).toBeGreaterThanOrEqual(400);
  });

  // GUARD for the headerless-read refusal below: the same request, the same
  ...
  test.failing(
    "refuses an unauthenticated read of every content row",
    async () => {
  ...
  it("still refuses the same write when a wrong Origin is supplied", async () => {
    // The control for the pair above: supplying a header the attacker was free
    // to omit is what gets them refused.
    const request = new NextRequest(
      `https://recopyfast.com/api/content/${SITE_ID}`,
      {
        method: "POST",
