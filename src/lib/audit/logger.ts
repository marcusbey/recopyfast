...
export class AuditLogger {
  ...
  async generateComplianceReport(params: {
    ...
    try {
      ...
      const reportData = {
        ...
        authentication_events: filteredLogs.filter(
          (log) => log.resource_type === "auth",
        ),
  ...
  private calculateComplianceMetrics(
    ...
    switch (reportType) {
      case "gdpr":
        metrics.data_subject_requests = logs.filter(
          (log) =>
            log.action.includes("data_export") ||
            log.action.includes("data_delete"),
        ).length;
      ...
      case "hipaa":
        metrics.phi_access_logs = logs.filter(
          (log) => log.metadata?.contains_phi === true,
        ).length;
