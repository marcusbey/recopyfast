...
export async function applyRateLimit(
  ...
  try {
    ...
    if (
      config.bypassRules?.paths?.some((path) =>
        context.endpoint.startsWith(path),
      )
    ) {
      return null;
    }
    ...
    const rateLimitConfig = createRateLimitConfig(
      ...
      context.identifierType === "api_key"
        ? "API_KEY_DEFAULT"
        : context.identifierType === "user"
          ? "USER_GENERAL"
          : "IP_GENERAL",
      context.endpoint,
    );
...
export async function validateDomain(
  ...
  try {
    ...
    const isEmbedRequest =
      context.endpoint.startsWith("/embed") ||
...
export async function securityMiddleware(
  ...
  try {
    ...
    const { sanitizedBody, error: sanitizationError } =
      await sanitizeRequestContent(context, config);
