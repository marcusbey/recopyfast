...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: events, error: eventsError } = await baseQuery.order(
      "created_at",
      { ascending: false },
    );
    ...
    const rateLimitEvents =
      events?.filter((e) => e.event_type === "rate_limit_exceeded") || [];
    const rateLimitStats = {
      total: rateLimitEvents.length,
      uniqueIPs: new Set(
        rateLimitEvents.map((e) => e.ip_address).filter(Boolean),
      ).size,
      uniqueEndpoints: new Set(
        rateLimitEvents.map((e) => e.endpoint).filter(Boolean),
      ).size,
    };
    ...
    if (!domainError && domainStats) {
      domainVerificationStats = {
        ...
        byMethod: domainStats.reduce((acc: Record<string, number>, domain) => {
          acc[domain.verification_method] =
            (acc[domain.verification_method] || 0) + 1;
          return acc;
        }, {}),
      };
    }
    ...
    if (!apiKeyError && apiKeyStats) {
      ...
      apiKeyStatsData = {
        ...
        recentlyUsed: apiKeyStats.filter(
          (k) => k.last_used_at && new Date(k.last_used_at) > oneWeekAgo,
        ).length,
      };
    }
