...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      new Date(existingVerification.expires_at) > new Date()
    ) {
      ...
      const verificationInstructions =
        sanitizedMethod === "dns"
          ? {
              type: "dns",
              record: generateDNSTXTRecord(
                existingVerification.verification_code,
              ),
            }
          : {
              type: "file",
    ...
    const verificationInstructions =
      sanitizedMethod === "dns"
        ? {
            type: "dns",
            record: generateDNSTXTRecord(verification.verificationCode),
          }
        : {
            type: "file",
            ...generateFileVerificationContent(verification.verificationCode),
          };
