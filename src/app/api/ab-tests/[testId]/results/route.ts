...
function calculateTestStatistics(
  ...
  const significanceResults = treatmentStats.map((treatment) => {
    ...
    return {
      ...
      lift:
        controlStats.conversion_rate > 0
          ? ((treatment.conversion_rate - controlStats.conversion_rate) /
              controlStats.conversion_rate) *
            100
          : 0,
  ...
  return {
    ...
    total_participants: new Set(results.map((r) => r.user_id || r.session_id))
      .size,
    test_duration_days: Math.ceil(
      (new Date().getTime() -
        new Date(
          Math.min(...results.map((r) => new Date(r.recorded_at).getTime())),
        ).getTime()) /
        (1000 * 60 * 60 * 24),
    ),
  };
